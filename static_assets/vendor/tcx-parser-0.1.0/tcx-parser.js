    ...
    parser.onclosetag = function (tagName) {
      if (tagName === "Activity") {
        ...
        currentWorkout.title =
          currentWorkout.type + " - " + datetime.toLocaleDateString();
      ...
      if (tag.name === "Activity") {
        ...
        if (currentLap && !currentTrack) {
          if (tag.name === "TotalTimeSeconds") {
            ...
            if (!isNaN(lapDuration)) currentLap.duration += parseFloat(tag.text);
          }
        }
      }
      currentTag = tag;
    ...
    parser.ontext = function (text) {
      if (currentTag) {
        if (currentTag.name === "Id") {
          if (!currentWorkout.title) currentWorkout.title = text;
        }
        if (currentTrack) {
          if (currentTrackpoint) {
            ...
            if (currentTag.name === "Time") {
              ...
              var KphToMps = function (kph) {
                return 0.277777778 * kph;
              };
              ...
              if (!currentTrackpoint.hr)
                if (!isNaN(parseInt(text))) currentTrackpoint.hr = parseInt(text);
            } else {
          ...
          if (currentTag.name === "TotalTimeSeconds") {
            ...
            if (!isNaN(lapDuration)) currentLap.duration += lapDuration;
          }
