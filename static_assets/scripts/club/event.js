function RCEvent(infoURL, clockURL, locale) {
	...
	class CompetitorSidebarDiv extends HTMLElement {
		constructor() {
			...
			if (!Object.hasOwn(competitor, "isColorDark")) {
	...
	function fetchCompetitorRoutes(cb) {
		...
		fetch(targetURL, {
			...
			.then((response) => {
				...
				if (zoomOnRunners && runnerPoints.length > 1) {
					map.fitBounds(L.latLngBounds(runnerPoints).pad(0.1), { maxZoom: 15 });
					zoomOnRunners = false;
				}
				computeSplitTimes();
				cb?.();
